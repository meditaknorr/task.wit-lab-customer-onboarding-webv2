...
const ScanVoterCardFront = () => {
    ...
    const [photo, setPhoto] = useState(state.find(storeProperty => (storeProperty.id === 3 && storeProperty.voterCardFront)));
    const language = (state.filter((data) => data.id === 1));
    const { appString } = useLocale(language[0].language);
    const router = useHistory();
    const overlayMode = 0;
    const voterCardFront = { id: 3, storePropertyName: 'voterCardFront' };
